...
class RinhaBackendCrebitosSimulation
  ...
  def randomTipoTransacao() = Seq("r", "d", "d")(Random.between(0, 2 + 1)) // not used
  ...
  val criteriosClientes = scenario("validações")
    ...
    .exec( // Consistencia do extrato
      ...
      .post("/clientes/#{id}/transacoes")
          .header("content-type", "application/json")
          .body(StringBody(s"""{"valor": 1, "tipo": "r", "descricao": "danada"}"""))
          .check(
            status.in(200),
  ...
  .exec(
    ...
    .exec(
      ...
      .post("/clientes/#{id}/transacoes")
          .header("content-type", "application/json")
          .body(StringBody(s"""{"valor": 1, "tipo": "r", "descricao": "123456789 e mais um pouco"}"""))
          .check(status.in(422, 400))
    )
    .exec(
      http("validações")
  ...
  /* 
    Separar recebíveis e débitos dá uma visão
  ...
  */
  setUp(
    validacaoTransacoesConcorrentes("d").inject(
      ...
      validacaoTransacoesConcorrentes("r").inject(
        atOnceUsers(validacaConcorrentesNumRequests)
      ).andThen(
